...
// Form dismissal event listeners
...
// Add event listeners for new dismissal options
if (formOverlay) {
...
async function handleUserMessage(e) {
    ...
    // Use a small timeout to give a more natural feeling before showing typing
    setTimeout(async () => {
        try {
            // Start request in background for faster response
            ...
            // Show typing indicator for at least 1 second to feel natural
            ...
            // Update form if data is available - with slight delay for smoother UX
            if (data.form_data) {
                setTimeout(() => {
                    updateFormDisplay(data.form_data, data.extracted_data);
                    formContainer.classList.add('active');
                    container.classList.add('form-active');
                }, 1000); // Short delay after message is complete
            }
...
// Add bot message to chat with typing animation for smoother experience
function addBotMessage(message) {
    ...
    // Split message by new lines and words for better animation
    ...
    lines.forEach(line => {
        words.push(...line.split(' ').map(word => word + ' '));
        words.push('<br>');
    });
    
    // Animation speed - adjust this for faster/slower typing
    const typingSpeed = 10; // milliseconds per character (lower = faster)
    
    // Function to animate text
    function animateText() {
        if (currentWord < words.length - 1) { // -1 to ignore last <br>
            if (words[currentWord] === '<br>') {
                contentDiv.innerHTML += '<br>';
            } else {
                contentDiv.innerHTML += words[currentWord];
            }
            currentWord++;
            scrollToBottom();
            
            // Calculate delay based on word length (faster for longer content)
            const delay = Math.max(10, typingSpeed * (words[currentWord] === '<br>' ? 1 : 1));
